...
main_weekly = Blueprint('main_weekly', __name__)
...
def process_weekly_report(project_name, report_date, sales_report, ad_report, fba_report):
    current_time = datetime.datetime.now().strftime('%H-%M-%S')
    source_folder = os.getcwd()
    os.chdir(source_folder)

    project_folder_path = os.path.join(source_folder, 'project', project_name, '周报')
    ...
    weekly_sales = pd.read_csv(sales_report_path, sep='\t', encoding='utf-8')
    weekly_ad_report = pd.read_excel(ad_report_path, engine='openpyxl')
    ...
    weekly_sales = weekly_sales.loc[weekly_sales['order-status'].isin(['Pending', 'Shipped', 'Unshipped'])]
    weekly_sales = weekly_sales.groupby(['sku'])[['quantity', 'item-price']].sum().reset_index()
    weekly_sales = weekly_sales.rename(columns={'sku': 'SKU', 'quantity': '订单量', 'item-price': '销售额'})

    ad_columns_keep = ['广告SKU', '广告ASIN', '展示量', '点击量', '花费', '7天总销售量(#)']
    weekly_ad_report = weekly_ad_report[ad_columns_keep]
    weekly_ad_report = weekly_ad_report.rename(columns={'广告SKU': 'SKU', '广告ASIN': 'ASIN', '花费': '广告花费',
                                                      '展示量': '曝光量', '7天总销售量(#)': '广告订单'})
    weekly_ad_report = (weekly_ad_report.groupby(['SKU'])
                       .agg({'曝光量': 'sum', '点击量': 'sum', '广告花费': 'sum', '广告订单': 'sum', 'ASIN': 'first'})
                       .reset_index())
    weekly_ad_report['单次点击花费'] = weekly_ad_report['广告花费'] / weekly_ad_report['点击量'] \
        if weekly_ad_report['点击量'].sum() > 0 else 0
    
    fba_columns_keep = ['sku', 'available']
    fba = fba[fba_columns_keep]
    fba = fba.rename(columns={'sku': 'SKU', 'available': '可售库存'})

    merged_data = pd.merge(weekly_sales, weekly_ad_report, on='SKU', how='outer')
    ...
    template_file = os.path.join('model_file', 'weekly_template.xlsx')

    project_weekly_file_path = os.path.join(project_folder_path, f'{project_name}_{report_date}_weekly_{current_time}.xlsx')
    ...
    workbook.save(project_weekly_file_path)

    with open(project_weekly_file_path, 'rb') as f:
        file_content = f.read()

    return file_content, f'{project_name}_{report_date}_weekly.xlsx'

@main_weekly.route('/weekly_report', methods=['GET', 'POST'])
def weekly_report():
    if request.method == 'POST':
        ...
        if not (project_name and report_date and sales_report and ad_report and fba_report):
            ...
            return redirect(url_for('main_weekly.weekly_report'))
        ...
        file_content, filename = process_weekly_report(project_name, report_date, sales_report, ad_report, fba_report)
